########################################################################################################################
# Script to save mjpeg data from one axis camera
# Script also calculates and saves the frame rate of a given camera.
# The streams from the camera will be analyzed for the time specified and average fps will be computed
# The results would be saved in "ip_address_of_camera".txt file
# The script expects the ip address of the camera as input. 
# Usage: python get_mjpeg_axis.py ip_address_of_the_camera
########################################################################################################################
...
def get_MJPEG_stream(ip_addr):
    ...
    try:
        # Stream data for time specified
