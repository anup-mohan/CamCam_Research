...
# The SIFT feature extraction code from OpenCV samples is run on each frame
...
# Usage: python get_utilization_mjpeg_me.py module_fname ip_fname out_fname fps loop_inc stream_time 
##############################################################################################################################
...
# Number of data points needed to find Nimax
MAX_DATA_POINTS = 4
...
def run_analysis_on_cameras(analysis_mod,ipfilename,resfilename,des_fps,STREAM_TIME):
	...
	num_data_points = 0 # counter to decide when to exit the test phase
	...
	while (i < length + 1):
		...
		# Change loop increment to 5 based on the cpu util value when trial = 1
		if (trial == 1):
			if (avg_top_cpu_util < 20.0):
				LOOP_INCREMENT = 5
...
if __name__ == '__main__':
	...
	run_analysis_on_cameras(analysis_mod,ipfilename,resfilename,des_fps,STREAM_TIME)
