...
def alloc_cams(inst_type,C,Nmax,N):
	...
	for i in range(len(inst_type)):
		if (Nmax[i] == 0):
			...
			inst_obj.append(inst_attr(inst_type[i],C[i],Nmax[i], C[i]/float(N)))
		else :
	...
	return (K1, rem_N,sorted_inst_obj[0].name,sorted_inst_obj[0].cost)	 
...
def apply_heuristic_solution(N,Nmax):
	...
	while (rem_N > 0):
		...
		K1, rem_N, inst_name, inst_cost = alloc_cams(inst_type,C,Nmax,rem_N)
	
		# Find the total cost
		total_cost += int(K1) * float(inst_cost)
	
		# Save Result
		result_obj.append(result_inst(inst_name,K1))
...
if __name__ == '__main__':
	...
	result_obj, total_cost = apply_heuristic_solution(N,Nmax)
